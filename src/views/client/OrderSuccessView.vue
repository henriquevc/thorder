  ...
  <div class="max-w-2xl mx-auto space-y-8 animate-in fade-in slide-in-from-top-4 duration-500">
    ...
    <div v-else class="space-y-6">
      ...
      <div class="text-center space-y-3">
        ...
        <div class="inline-block px-5 py-1.5 bg-primary-100 border border-primary-200 rounded-full font-mono text-sm font-bold text-primary shadow-inner mt-2">
      ...
      <Card class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        <CardHeader class="pb-3 border-b border-slate-100 bg-slate-50/50">
          <CardTitle class="text-base font-bold text-slate-700 uppercase tracking-wider">Dados da Entrega &amp; Contato</CardTitle>
        </CardHeader>
        <CardContent class="p-6 space-y-4 text-sm md:text-base text-slate-800">
          <!-- Nome e Contato -->
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="flex items-center gap-2.5">
              <User class="w-4.5 h-4.5 text-primary shrink-0" />
              <span><strong class="text-slate-600 font-semibold">Cliente:</strong> {{ order.customer_name }}</span>
            </div>
            <div class="flex items-center gap-2.5">
              <Mail class="w-4.5 h-4.5 text-primary shrink-0" />
              <span><strong class="text-slate-600 font-semibold">E-mail:</strong> {{ order.customer_email }}</span>
            </div>
            <div class="flex items-center gap-2.5">
              <Phone class="w-4.5 h-4.5 text-primary shrink-0" />
              <span><strong class="text-slate-600 font-semibold">Celular:</strong> {{ order.customer_phone }}</span>
            </div>
            <div class="flex items-center gap-2.5">
              <Truck class="w-4.5 h-4.5 text-primary shrink-0" />
              <span><strong class="text-slate-600 font-semibold">Entrega:</strong> {{ order.shipping_carrier }}</span>
            </div>
            <div class="flex items-center gap-2.5 md:col-span-2">
              <Wallet class="w-4.5 h-4.5 text-primary shrink-0" />
              <span><strong class="text-slate-600 font-semibold">Forma de Pagamento:</strong> {{ getPaymentLabel(order.payment_method) }}</span>
            </div>
          </div>
          ...
          <div class="flex items-start gap-2.5">
            <MapPin class="w-4.5 h-4.5 text-primary shrink-0 mt-0.5" />
            <div class="space-y-1">
              <span><strong class="text-slate-600 font-semibold">Endereço Completo:</strong></span>
              <p class="text-slate-800 text-sm md:text-base leading-relaxed font-medium">{{ order.shipping_address }}</p>
              <p class="text-xs text-slate-500 font-mono mt-0.5">CEP: {{ order.shipping_cep }}</p>
            </div>
          </div>

          <!-- Informações de Troco em Dinheiro -->
          <div v-if="order.payment_method === 'dinheiro' && order.change_amount && order.change_amount > 0" 
            class="flex items-start gap-3 p-4 bg-amber-500/5 border border-amber-500/10 rounded-xl text-sm text-amber-800 animate-in fade-in duration-300"
          >
            <Coins class="w-5 h-5 shrink-0 text-amber-600 mt-0.5" />
            <div class="space-y-1">
      ...
      <Card class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        <CardHeader class="pb-3 border-b border-slate-100 bg-slate-50/50 flex flex-row items-center justify-between">
          <CardTitle class="text-base font-bold text-slate-700 uppercase tracking-wider">Itens Comprados</CardTitle>
          <span class="text-xs text-slate-500 font-bold uppercase">{{ formatDate(order.created_at) }}</span>
        </CardHeader>
        <CardContent class="p-6 space-y-4">
          <!-- Lista dos itens -->
          <div class="divide-y divide-slate-100 text-sm md:text-base">
            <div 
            ...
            >
              ...
              <span class="font-black text-slate-900">{{ formatPrice(item.price * item.quantity) }}</span>
            </div>
          </div>
          ...
          <div class="space-y-3 text-sm md:text-base">
            <div class="flex justify-between text-slate-600">
              ...
              <span class="font-semibold text-slate-800">{{ formatPrice(order.items_cost) }}</span>
            </div>
            <!-- Cupom de Desconto -->
            <div v-if="order.coupon_code" class="flex justify-between text-red-650 font-semibold animate-in fade-in duration-300">
            ...
            <div class="flex justify-between text-slate-600">
              ...
              <span class="font-semibold text-slate-800">{{ formatPrice(order.shipping_cost) }}</span>
            </div>
            
            <div class="border-t border-slate-100 my-2"></div>
            
            <div class="flex justify-between items-baseline">
              <span class="font-bold text-slate-700 text-base md:text-lg">Valor Total</span>
      ...
      <Card v-if="pixEnabled && pixKey && (!order.payment_method || order.payment_method === 'pix')" class="bg-white border border-amber-200 rounded-2xl shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-3 duration-500">
        <CardHeader class="pb-3 border-b border-amber-100 bg-amber-50/50 flex flex-row items-center gap-2">
          ...
          <CardTitle class="text-base font-bold text-amber-800 uppercase tracking-wider">Pagamento via Pix</CardTitle>
        </CardHeader>
        <CardContent class="p-6 flex flex-col items-center gap-4 text-center">
          <!-- Temporizador de Expiração -->
          <div class="px-4 py-1.5 rounded-full border text-xs font-bold tracking-wide uppercase inline-flex items-center gap-2"
            :class="isExpired 
              ? 'bg-red-500/10 border-red-500/20 text-red-650' 
              : 'bg-amber-500/10 border-amber-500/20 text-amber-600 animate-pulse'"
          >
            <Clock class="w-4 h-4" :class="{'text-red-500': isExpired, 'text-amber-500 animate-spin': !isExpired}" />
            <span>{{ isExpired ? 'QR Code Expirado' : `Expira em: ${countdownText}` }}</span>
          </div>

          <p v-if="!isExpired" class="text-sm text-slate-600 max-w-sm leading-relaxed">
          ...
          <div v-else class="p-4.5 rounded-xl border border-red-250 bg-red-50/50 text-sm text-red-800 text-left max-w-md animate-in fade-in duration-300 space-y-1.5">
            <h5 class="font-extrabold flex items-center gap-2 text-red-900 text-sm">
              <Clock class="w-4 h-4 text-red-600 shrink-0" />
              Tempo Limite Excedido
            </h5>
            <p class="text-red-700 leading-relaxed text-xs">
              O tempo limite de 10 minutos para pagamento online via QR Code expirou. Para dar andamento ao seu pedido, clique no botão de <strong>WhatsApp</strong> abaixo para enviar os detalhes e combinar o pagamento diretamente com a loja.
            </p>
          </div>
          ...
          <div class="relative p-3 bg-white border-2 border-slate-100 rounded-2xl shadow-inner group overflow-hidden">
            ...
            <div v-else class="absolute inset-0 bg-white/90 backdrop-blur-xs flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-2xl">
              <span class="text-sm font-bold text-slate-700 bg-slate-100 border px-3.5 py-2 rounded-xl shadow-sm">Escaneie pelo App do Banco</span>
            </div>
          </div>

          <!-- Valor e Detalhes da Conta -->
          <div class="space-y-1">
            <span class="text-xs text-slate-500 font-bold uppercase tracking-wider">Valor do Pix</span>
            <div class="text-2xl md:text-3xl font-black text-amber-850 leading-tight" :class="isExpired ? 'text-slate-450 line-through' : 'text-amber-850'">
              {{ formatPrice(order.total_cost) }}
            </div>
            <div class="text-xs text-slate-600 flex items-center justify-center gap-1.5 mt-1">
              <span>Beneficiário:</span>
              <strong class="text-slate-800 font-bold">{{ pixName || 'LOJA THORDER' }}</strong>
            </div>
          </div>

          <!-- Botões Copia e Cola -->
          <div class="w-full max-w-md pt-2 space-y-2.5">
            <!-- Botão Copiar -->
            <Button 
              variant="outline"
              class="w-full rounded-xl py-5.5 border-amber-300 bg-amber-50/30 hover:bg-amber-50 text-amber-900 font-bold text-sm md:text-base flex items-center justify-center gap-2 transition-transform"
              :disabled="isExpired"
              ...
              @click="handleCopyPix"
            >
              <component :is="isCopied ? Check : Copy" class="w-4.5 h-4.5 animate-in zoom-in duration-200" :class="isExpired ? 'text-slate-400' : 'text-amber-700'" />
            ...
            <div 
              v-if="!isExpired" 
              class="p-2.5 border border-dashed border-slate-200 bg-slate-50 rounded-xl text-xs font-mono text-slate-600 truncate text-left px-3 select-all"
            >
              {{ getPixPayloadString() }}
            </div>
          </div>
      ...
      <div class="p-6 bg-emerald-50 border border-emerald-100 rounded-2xl flex flex-col items-center gap-3.5 text-center shadow-sm">
        <div class="text-sm md:text-base text-emerald-850 font-semibold max-w-md leading-relaxed">
        ...
        <Button 
          class="rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold px-8 py-6 text-base shadow-lg shadow-emerald-500/20 gap-2.5 flex items-center justify-center cursor-pointer transition-transform hover:scale-[1.02]"
        ...
        <p v-if="!pixEnabled" class="text-xs text-emerald-700 font-medium animate-pulse">
      ...
      <div class="flex items-center gap-3 justify-center pt-2">
        <Button 
          variant="outline"
          class="rounded-xl border-slate-300 hover:bg-slate-100 text-slate-700 font-bold px-6 py-5 text-sm"
